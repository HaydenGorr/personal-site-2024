...
export default function AiResponseChatbox( { textToDisplay, largeBox=true, minimiseResponseBox, ai_response_error } ) {
    ...
    return (
        <div className={`ai-response-chatbox h-full w-full max-w-prose transition-all ease-in-out duration-300`}>
            ...
            <div className={`ai-response-box prose px-4 rounded-md overflow-scroll min-h-12 md:max-h-[calc(100vh-33rem)] max-h-[calc(100vh-29rem)] ${ai_response_error ? 'bg-dr-100': 'bg-dg-100'} ${textToDisplay && !minimiseResponseBox ? 'h-fit opacity-100' : 'h-10 opacity-15'} ${largeBox ? 'h-fit' : 'max-h-12'}`}>
