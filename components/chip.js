...
export default function Chip({chip_text, add_keywords_to_filter = () => {}, index=0, disble_border=false }) {
    ...
    return (
        <div key={index} className={`flex my-1 mr-1 px-1 py-1 text-xs font cursor-pointer ${disble_border ? '' : 'border-2 rounded-md border-black'}`} onClick={() => add_keywords_to_filter(chip_text)}>
